...
# Ensure util_train2 exists or paste the add_rolling_features function here
from util_train2 import add_rolling_features
...
# Refined Model (Only strictly needed if USE_REFINED_MODEL is True, but loaded for safety)
try:
    ...
    state_2_model = None
    state_2_features = []
    # If the model features file is missing but we want to try anyway:
    if not state_2_features:
        base = ['in_bed__inBed', 'asleep__Asleep', 'asleep__Awake', 
                'state__Core Sleep', 'state__Deep Sleep', 'state__REM']
        state_2_features = base + [f"{c}_roll_mean" for c in base] + [f"{c}_roll_std" for c in base]
...
probability_buffer = deque(maxlen=12)
...
def classify_snippet(snippet, use_refined=True):
    ...
    if use_refined and state_2_model is not None:
        try:
            ...
            df_refined = add_rolling_features(df_probs)
            ...
            print(f"Error in refined model: {e}. Falling back to base model.")

    # OPTION 2: Use Base Model (Training Data Generation)
    # This runs if USE_REFINED_MODEL is False OR if the refined model fails
